...
- **No conversation memory.** Each message is independent, so "move that to
  3pm" won't resolve "that". Say "move the glossary review to 3pm" instead.
  To add memory you'd need external state (Vercel KV, Upstash Redis) keyed by
  chat id, storing the last few turns.
