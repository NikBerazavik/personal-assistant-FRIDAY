...
import { today, addDays, startOfDay, startOfWeek, normalizeDate, formatForDisplay } from "./dates.js";
...
export function flattenTask(page) {
  ...
  return {
    ...
    // only present when not-empty because omitted key costs no input
...
function scopeFilter(scope, { date_from, date_to } = {}) {
  ...
  switch (scope) {
    ...
    case "this_week": {
      // Monday-sunday week containing today and starting from today
      // days past are in overdue scope
    ...
    case "overdue":
      return { and: [{ property: P.date, date: { before: startOfDay(t) } }, notDone] };
    case "unscheduled":
      return { and: [{ property: P.date, date: { is_empty: true } }, notDone] };
    case "all_open":
    default:
      return notDone;
  }
...
export async function updateTask({ page_id, status, date_start, date_end, priority, domain, notes }) {
  const properties = {};
