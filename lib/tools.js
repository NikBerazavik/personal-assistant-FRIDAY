  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        notes: { 
          type: "string",
          description: "Any detail with no dedicated field: location, building, floor, room, meeting link, attendees, agenda. If the user says WHERE something happens, it belongs here."
        },
      },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        priority: { type: "string", enum: config.values.priority },
        domain: { type: "string", enum: config.values.domain },
        notes: { type: "string", description: "Any detail with no dedicated field: location, building, floor, room, meeting link, attendees, agenda. If the user says WHERE something happens, it belongs here."}
      },
      required: ["page_id"],
    },
  },
  ...
  try {
    switch (name) {
      case "get_tasks": {
        ...
        console.log(`get_tasks returned ${tasks.length}:`, JSON.stringify(tasks.map(t => ({ name: t.name, date: t.date, status: t.status }))));
        return { tasks };
      }
      case "create_task":
